...
def main():
    ...
    while upper_est_ore < max_ore:
        upper_est *= 22
    print(
        f"Lower est {lower_est} uses {lower_est_ore}: Upper est {upper_est} uses {upper_est_ore}"
    )
    while True:
...
def ore_for_fuel(amt, recipes):
    ...
    while not finished:
        ...
        required = [
            Qty_Type(next_required[ingredient], ingredient)
            for ingredient in next_required.keys()
        ]
